...
def evaluate_model(model, dataloader, device):
    ...
    with torch.no_grad():
        for inputs, labels in dataloader:
            ...
            # Combine the first two classes by taking the maximum probability
            outputs[:, 0] = torch.max(outputs[:, 0], outputs[:, 1])
            outputs = torch.cat([outputs[:, :1], outputs[:, 2:]], dim=1)
            
            _, preds = torch.max(outputs, 1)
            
            # Combine labels 1 into 0, 2 and 3 into 1 and 2 respectively
            labels = torch.where(labels == 1, torch.tensor(0, device=device), labels)
            labels = torch.where(labels == 2, torch.tensor(1, device=device), labels)
            labels = torch.where(labels == 3, torch.tensor(2, device=device), labels)
...
def plot_confusion_matrix(cm, classes, model_name, save_path, average=False, font_size=18):
    ...
    if average:
        sns.heatmap(cm, annot=True, fmt='.2f', cmap='Blues', xticklabels=classes, yticklabels=classes, annot_kws={"size": font_size}, vmax=100, vmin=0)
...
if __name__ == "__main__":
    ...
    for model_name in model_names:
        ...
        for train_index, val_index in kf.split(patient_data):
            ...
            # Log metrics
            with open(os.path.join(save_dir, f'{model_name}_3classes.txt'), 'a') as f:
                f.write(f'Fold: {fold}\n')
            ...
            plot_confusion_matrix(cm, classes, model_name, os.path.join(save_dir, f'{model_name}_confusion_matrix_fold_{fold}_3classes.png'))
        ...
        plot_confusion_matrix(avg_cm_percentage, classes, model_name, os.path.join(save_dir, f'{model_name}_average_confusion_matrix_3classes.png'), average=True)
